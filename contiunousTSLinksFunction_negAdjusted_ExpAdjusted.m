...
function[] = contiunousTSLinksFunction_negAdjusted_ExpAdjusted(tissue, s, n)
    ...
    for i = 1:53
        ...
        meanDS = mean(dsMat');
        qThr = quantile(meanDS, 1/3);
        inGenes = meanDS > qThr;
        finalInGenes = inGenes(expGenesInd);
        expTemplate = (finalInGenes' + 0) * (finalInGenes + 0);
    ...
    toc
    fileName = sprintf('~/networks/continuousTSLinksResults/%s_%d_%d_NegAdjusted_ExpAdjusted.mat', ...
                       tissue, s, n)
    save(fileName, 'space');
end
